-- This is a modified version of the base game data-update which auto barrels all fluids
-- The difference is that fluids which do not have the flag auto_barrel set will only get a fill barrel recipe and not an empty barrel recipe
-- For the sake of this mod, newly created filled barrel items are created as tool directly
-- Additionally we will fill the fluid mapping table
...
local function generate_barrel_recipe_icons(fluid, base_icon, side_mask, top_mask, fluid_icon_shift)
    ...
    elseif fluid.icons then
        icons = util.combine_icons(icons, fluid.icons, {
            scale = 0.5,
            shift = fluid_icon_shift
        }, fluid.icon_size)
    end
...
local function create_fill_barrel_recipe(item, fluid)
    ...
    local recipe = {
        ...
        order = fluid.order,
        enabled = false,
        icons = generate_barrel_recipe_icons(fluid, barrel_fill_icon, barrel_fill_side_mask, barrel_fill_top_mask,
            {-8, -8}),
        ingredients = {{
            type = "fluid",
            name = fluid.name,
...
local function process_fluid(fluid, technology, empty_barrel_item)
    ...
    -- Get this fluid's amount for this barrelling recipe
    local fluid_amount
    for _, ingredient in pairs(barrel_fill_recipe.ingredients or {}) do
